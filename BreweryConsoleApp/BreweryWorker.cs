...
namespace BreweryConsoleApp
{
    class BreweryWorker
    {
        ...
        private const int NUMBER_OF_FILLING_TOPPING_THREADS = 6;
        ...
        // tracing
        private TraceSource _trace = Program.TRACE;
        ...
        public void Start()
        {
            _trace.TraceEvent(TraceEventType.Information, 1, "Simulation Started");
            ...
            Task.Run(() =>
            {
                _trace.TraceEvent(TraceEventType.Information, 1, $"{NUMBER_OF_WASHING_THREADS} washing threads started");
            ...
            Task.Run(() =>
            {
                _trace.TraceEvent(TraceEventType.Information, 1, $"{NUMBER_OF_FILLING_TOPPING_THREADS} filling threads started");
            ...
            Task.Run(() =>
            {
                _trace.TraceEvent(TraceEventType.Information, 1, $"{NUMBER_OF_FILLING_TOPPING_THREADS} topping threads started");
            ...
            Task.Run(() =>
            {
                _trace.TraceEvent(TraceEventType.Information, 1, $"{NUMBER_OF_PACKING_THREADS} packing threads started");
            ...
            while (countedBoxes <= expectedNoOfBoxes)
            {
                ...
                _trace.TraceEvent(TraceEventType.Information, 1, $"Box no {++countedBoxes} received with {box.Count} bottles");
            }
            _trace.TraceEvent(TraceEventType.Information, 1, "Simulation ended");

            // nice close of all tracing
            foreach (TraceListener tl in _trace.Listeners)
            {
                tl?.Close();
            }
        ...
        private void BoxingBottle(int nr)
        {
            ...
            _trace.TraceEvent(TraceEventType.Information, 1, $"{myName} is started");
        ...
        */
        ...
        private void TraceBottle(Bottle b)
        {
            ...
            {
                _trace.TraceEvent(TraceEventType.Information, 1, "Sampling " + b);
            }
        }
    }
}